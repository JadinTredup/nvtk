...
class eegBaseline(object):
    ...
    Simple experiment for recording EEG baseline signal
        ...
        10  second end
    """
    def __init__(self, resolution=None, save_fn=None):
        pygame.init()
        pygame.font.init()
        if resolution==None:
        ...
        if save_fn is not None:
            LOG_FILE = os.path.join('Data', save_fn)
        else:
            LOG_FILE = os.path.join('Data', 'temporary.txt')

    def Run(self):
        """
        Main loop for the experiment
        """
        self.eeg.startEEGStreaming(save_fn=LOG_FILE)
        self.running = True
        while self.running:
